 ...
 */
const useLocalCurrentSong = (): { song?: ISong; error?: Error } => {
  ...
  useEffect(() => {
    ...
    const listener = Player.addListener('onLocalCurrentSongChange', (state: IPlaybackState) => {
      setError(undefined);
...
export default useLocalCurrentSong;
