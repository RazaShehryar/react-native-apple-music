...
class MusicModule: RCTEventEmitter {
    ...
    func fetchDetails(for song: Song) async -> Song? {
        ...
        do {
            ...
            guard let catalogSong = catalogSongs.first else { return nil }
            return catalogSong
        } catch {
            print("Error fetching details for song \(song.id): \(error)")
            return nil
        }
    }
    
    @available(iOS 16.0, *)
    @objc(getUserLibrary:resolver:rejecter:)
    func getUserLibrary(options: NSDictionary, resolver resolve: @escaping RCTPromiseResolveBlock, rejecter reject: @escaping RCTPromiseRejectBlock) {
        Task {
            ...
            do {
                ...
                let tasks = await withTaskGroup(of: Song?.self) { group -> [Song] in
                    for song in response.items {
                        group.addTask {
                            if let result = await self.fetchDetails(for: song) {
                                return result
                            } else {
                                return nil
                            }
                ...
                print("Transformed songs list: \(tasks)")
                
                let songs = tasks.compactMap(convertSongToDictionary)
